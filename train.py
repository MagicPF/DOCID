...
def train(x_target, x_ref, y_ref, epoch_num):
    ...
    for epochnumber in range(epoch_num):
        ...
        y_r = np.array(y_r)
        print("milestone1")
        print(int(len(x_target) / batchsize))
        # print("x_target:{} , batchsize:{}".format(x_target,batchsize))
        for i in range(int(len(x_target) / batchsize)):
            ...
            print("target class batch finished")
        ...
        print("epoch : {} ,Descriptive loss : {}, Compact loss : {}".format(epochnumber+1, loss[epochnumber], loss_c[epochnumber]))
        if epochnumber % 10 == 0:
